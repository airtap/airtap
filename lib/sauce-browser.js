...
SauceBrowser.prototype._stop = function (err, cb) {
  clearInterval(this._pingTimer)
  this._pingTimer = null

  if (this._webdriver) {
    this._setJobStatus(err, this.stats.ok, () => {
...
SauceBrowser.prototype._setJobStatus = function (err, ok, callback) {
  // If an error occurred we cannot safely set the status
  if (err) return process.nextTick(callback)

  this._webdriver.sauceJobStatus(ok, (err) => {
    if (err) this.debug('setting Sauce Labs job status failed: %O', cleanError(err))
    callback()
  })
}
