...
Zuul.prototype._setup = function (cb) {
  ...
  var server = control_app(config).listen(0, function () {
    cb(null, server)
  })
}
...
Zuul.prototype.run = function (cb) {
  ...
  self._setup(function (err, server) {
    if (err) return cb(err)

    debug('control server active on port %d', server.address().port)
    config.control_port = server.address().port
