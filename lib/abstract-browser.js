...
function AbstractBrowser (config) {
  if (!(this instanceof AbstractBrowser)) {
    return new AbstractBrowser(config)
  }
...
AbstractBrowser.prototype.run = function (url, opts, callback) {
  if (this._status !== 'stopped') throw new Error('Cannot run until stopped')
...
AbstractBrowser.prototype.start = function (url) {
  if (this._status !== 'stopped') throw new Error('Cannot start until stopped')

  this._status = 'starting'
  this.reset()
  this.emit('starting', url)
...
AbstractBrowser.prototype.reset = function () {
  if (this._status !== 'starting' && this._status !== 'started') return
...
AbstractBrowser.prototype.stop = function (err, callback) {
  ...
  this._stopWithTimeout((err2) => {
    this._status = 'stopped'
    this.emit('stop', err || err2 || null, this.stats)
  })
}

AbstractBrowser.prototype._stopWithTimeout = function (callback) {
  let called = false
...
AbstractBrowser.prototype._stop = function (callback) {
  process.nextTick(callback)
}
